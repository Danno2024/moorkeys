...
<?php endif; ?>
                    ...
                    <nav class="hidden md:flex items-center gap-6">
                        ...
                        <a href="<?php echo e(route('page', $p->slug)); ?>" class="text-gray-600 hover:text-gray-900 text-sm font-medium"><?php echo e($p->title); ?></a>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php if(auth()->guard()->check()): ?>
                            <a href="<?php echo e(route('dashboard')); ?>" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Dashboard</a>
                        <?php else: ?>
                            <a href="<?php echo e(route('login')); ?>" class="text-gray-600 hover:text-gray-900 text-sm font-medium">Login</a>
                            <a href="<?php echo e(route('register')); ?>" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Get Started</a>
                        <?php endif; ?>
                    </nav>
                ...
                <div :class="{ 'block': menuOpen, 'hidden': !menuOpen }" class="md:hidden pb-4 border-t border-gray-100 pt-4">
                    <div class="flex flex-col gap-3">
                        ...
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php if(auth()->guard()->check()): ?>
                            <a href="<?php echo e(route('dashboard')); ?>" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-center text-sm font-medium">Dashboard</a>
                        <?php else: ?>
                            <a href="<?php echo e(route('login')); ?>" class="text-gray-600 hover:text-gray-900 text-sm font-medium">Login</a>
                            <a href="<?php echo e(route('register')); ?>" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-center text-sm font-medium">Get Started</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-28">
                <div class="max-w-4xl mx-auto text-center">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <?php if(auth()->guard()->check()): ?>
                            <a href="<?php echo e(route('dashboard')); ?>" class="w-full sm:w-auto px-8 py-3 bg-white text-indigo-700 rounded-lg font-semibold hover:bg-indigo-50 text-center">Go to Dashboard</a>
                        <?php else: ?>
                            <a href="<?php echo e(route('register')); ?>" class="w-full sm:w-auto px-8 py-3 bg-white text-indigo-700 rounded-lg font-semibold hover:bg-indigo-50 text-center">Start Free Trial</a>
        ...
        <section id="pricing" class="py-16 sm:py-20 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 max-w-5xl mx-auto">
                    ...
                    <?php $isPopular = $loop->iteration === 2; ?>
                    <div class="rounded-xl p-6 sm:p-8 <?php echo e($isPopular ? 'bg-gray-900 text-white ring-2 ring-indigo-500 lg:scale-105' : 'bg-white border border-gray-200 text-gray-900'); ?> shadow-sm">
                        <h3 class="text-xl font-bold <?php echo e($isPopular ? 'text-white' : 'text-gray-900'); ?>"><?php echo e($plan->name); ?></h3>
                        <p class="<?php echo e($isPopular ? 'text-gray-400' : 'text-gray-500'); ?> mt-2 text-sm"><?php echo e($plan->description); ?></p>
                        <p class="mt-6 flex items-baseline gap-1">
                            <span class="text-4xl font-bold">$<?php echo e(number_format($plan->price, 0)); ?></span>
                            <span class="text-sm <?php echo e($isPopular ? 'text-gray-400' : 'text-gray-500'); ?>">/<?php echo e($plan->billing_period); ?></span>
                        </p>
                        <ul class="mt-8 space-y-3">
                            <li class="flex items-center gap-2 text-sm"><svg class="w-5 h-5 <?php echo e($isPopular ? 'text-indigo-400' : 'text-indigo-600'); ?> shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> <?php echo e($plan->max_keys > 0 ? $plan->max_keys : 'Unlimited'); ?> Keys</li>
                            <?php if($plan->features->count()): ?><?php $__currentLoopData = $plan->features; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feature): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li class="flex items-center gap-2 text-sm"><svg class="w-5 h-5 <?php echo e($isPopular ? 'text-indigo-400' : 'text-indigo-600'); ?> shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> <?php echo e($feature->name); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?> <?php endif; ?>
                        </ul>
                        <?php if($plan->price == 0): ?>
                            <a href="<?php echo e(route('register')); ?>" class="mt-8 block w-full py-3 text-center rounded-lg font-semibold text-sm <?php echo e($isPopular ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'); ?>">Get Started Free</a>
                        <?php elseif($plan->stripe_price_id && auth()->check()): ?>
                            <form method="POST" action="<?php echo e(route('checkout.session', $plan)); ?>">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="mt-8 block w-full py-3 text-center rounded-lg font-semibold text-sm <?php echo e($isPopular ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'); ?>">Subscribe Now</button>
                            </form>
                        <?php elseif($plan->stripe_price_id): ?>
                            <a href="<?php echo e(route('register')); ?>" class="mt-8 block w-full py-3 text-center rounded-lg font-semibold text-sm <?php echo e($isPopular ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'); ?>">Get Started</a>
                        <?php else: ?>
        ...
        <footer class="bg-gray-900 text-gray-400 py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap justify-center gap-6 mb-6">
                    ...
                    <a href="<?php echo e(route('page', $p->slug)); ?>" class="text-gray-400 hover:text-white text-sm"><?php echo e($p->title); ?></a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
                <?php endif; ?>
                <div class="text-center space-y-2">
                    ...
                    <?php if(config('app.footer_text')): ?>
                        <p class="text-sm"><?php echo e(config('app.footer_text')); ?></p>
                    <?php endif; ?>
                </div>
            </div>
        </footer>
...
<?php /**PATH D:\dev\htdocs\moorkeys\resources\views/home.blade.php ENDPATH**/ ?>